"""Collect and format the program configuration from commandline or a dict."""
...
def get_program_config(
    ...
    Args:
        program_config: A program configuration dict. If not given, commandline arguments are
            parsed.
        additional_args: List of custom commandline arguments.
    ...
    if program_config is None:
        ...
        parser.add_argument('-f', '--config-file', help='Configuration yaml file. The following'
                            ' command-line options are ignored when a configuration file is'
                            ' present: --name, --base-dir, --instance, --backend, --qubits,'
                            ' --calibrations.',
                            metavar='PATH', dest='config_file')
        parser.add_argument('-w', '--work-dir', help='Scratch working directory. The contents of'
                            ' the directory are removed before the program execution.',
                            metavar='PATH', dest='work_dir')
        parser.add_argument('-n', '--name', help='Identifier for this program instance.',
                            metavar='NAME', dest='name')
        parser.add_argument('-d', '--base-dir', help='Base data directory path.',
                            metavar='BASE_DIR', dest='base_dir')
        ...
        parser.add_argument('-c', '--calibrations', nargs='?', const='', help='Load calibrations.'
                            ' If a path is given, data is loaded from '
                            'BASE_DIR/PATH/parameter_values.csv. If the path ends with .csv, it'
                            ' is considered to be the path to the CSV file (absolute if it starts'
        ...
        if options.config_file is not None:
            with open(options.config_file, 'r', encoding='utf-8') as source:
