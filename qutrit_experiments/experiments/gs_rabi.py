from typing import Optional, Sequence, Iterable, List, Tuple, Union
import numpy as np
import scipy
import lmfit
...
from qiskit_experiments.framework import BaseExperiment, Options, ExperimentData, BackendData
...
class GSRabi(BaseExperiment):
    ...
    def __init__(
        ...
        if time_unit is None and backend is not None:
            time_unit = BackendData(backend).dt
    ...
    def _pre_circuit(self) -> QuantumCircuit:
        ...
        if initial_state == 'x':
            ...
            # Qiskit thinks the pulse envelope is modulated with exp(-iωt) and therefore ShiftPhase(φ)
            # effects a physical Rz(-φ). The scheduler thus translates rz(-φ) as ShiftPhase(φ).
    ...
    def _generate_fit_guesses(
        ...
        if fixed_freq_sign:
            ...
            for user_opt in user_opts:
                negative_freq_opt = user_opt.copy()
